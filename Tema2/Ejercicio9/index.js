...
Vue.component('home', {
    template: `
    <div><h1>Mis tareas<input type="text" placeholder="Filtro" v-model="filter"></h1><tasks :filter="filter"></tasks></div>
    `,
    data: function() {
        return {
            filter: ''
...
Vue.component('task', {
    ...
    {{ data.title }}
    <a href="#" @click.stop="$emit('remove', data)">Delete</a>
    </li>`,
    props: ['data']
});
...
Vue.component('tasks', {
    template: `
    <ul>
    <li @click="$router.push('/add')">+ Add task</li>
    <task v-for="t in doFilter()" :data="t"
    @remove="doRemove"
    @click.native="$router.push('/edit/' + t.id)">
    </task>
    </ul>`,
    props: ['filter'],
    data: function() {
